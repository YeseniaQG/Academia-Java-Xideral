...
public class ConcurrentScraper {
    ...
    public ConcurrentScraper(int threadCount) {
        // Inicializamos el pool de hilos con el tamaño deseado
    ...
    public List<PageResult> fetchAll(List<String> urls) {
        // Creamos un CompletableFuture por cada URL para que corran en paralelo
        List<CompletableFuture<PageResult>> futures = urls.stream()
            .map(url -> CompletableFuture.supplyAsync(
                () -> fetchPage(url), executor)) // Usamos nuestro pool de hilos
            .collect(Collectors.toList());

        // Esperamos a que todos terminen con allOf
        CompletableFuture.allOf(
            futures.toArray(new CompletableFuture[0])).join();

        // Recolectamos los resultados usando join() (que ya no bloquea porque allOf terminó)
    ...
    public List<PageResult> fetchWithTimeout(List<String> urls, long timeoutMs) {
        List<CompletableFuture<PageResult>> futures = urls.stream()
            .map(url -> CompletableFuture.supplyAsync(
                ...
                // Fallback: si tarda más de timeoutMs, devuelve un resultado de error 408
                .completeOnTimeout(
    ...
    public void shutdown() {
        // Siempre es vital cerrar el executor para que el programa termine correctamente
        executor.shutdown();
    }
